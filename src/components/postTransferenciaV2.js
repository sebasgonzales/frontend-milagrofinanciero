...
function PostTransferenciaV2() {
  ...
  const realizarTransaccion = async () => {
    try {
      ...
      // Después de realizar la transferencia con éxito, muestra una notificación
      await toast.success('Transferencia realizada con éxito');

      // Espera unos segundos antes de recargar la página
      setTimeout(() => {
        window.location.reload();  // Recarga la página
      }, 5000);  // 3000 milisegundos = 3 segundos (ajusta según tus necesidades)
  ...
  <Form className="labelPersonalizado" noValidate validated={validated} onSubmit={handleSubmit}>
    ...
    <div className="botonesAlPie mb-2">
      ...
      >Transferir
       ...
       {/* Componente de Toast para mostrar notificaciones */}
  <ToastContainer />
    </div>
